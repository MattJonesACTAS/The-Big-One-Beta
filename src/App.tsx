...
const DOSE_CONFIG: Record<string, { doses: DoseOption[], customUnit?: string }> = {
  'Adrenaline push': { 
    doses: [
  ...
  'Magnesium': { 
    doses: [
      { dose: '2.5g', population: 'adult', indication: 'pVT secondary to prolonged QT / Torsades de pointes / Pre-eclampsia / Seizures due to eclampsia' },
      { dose: '50mg/kg', population: 'paed', indication: 'With cardiac output / Without cardiac output', calculated: true },
      { dose: 'Other', population: 'both' }
    ] 
  },
  'Midazolam push': { 
...
function PHEASelection({ checkedItems, onToggle }: { checkedItems: string[], onToggle: (label: string) => void }) {
  return (
    <div className="h-full pb-10">
      <SectionGroup title="INITIAL TEAM BRIEF" color="purple" items={['Adequate hands and skills mix?', { label: 'Optimise patient position', subItems: ['Consider relocating patient now to optimal location', 'If remaining on scene and resources allow, prepare extrication now', 'Apply C-spine immobilisation if required'] }, { label: 'Assign roles', subItems: ['Team leader', 'Airway primary', 'Airway assistant', 'Drugs & access primary', 'Drugs & access assistant', 'Gofer'] }]} checkedItems={checkedItems} onToggle={onToggle} />
